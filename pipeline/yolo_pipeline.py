...
def _draw_result(img: np.ndarray, result: DetectionResult) -> np.ndarray:
        ...
        # ── Add this summary overlay block ──────────────────────────
        STATUS_COLORS = {
            "OK":       (80,  200,  80),
            "LOW":      (0,   180, 255),
            "CRITICAL": (0,     0, 255),
        ...
        summary = [
            ...
            (result.water.status,         f"Water: {result.water.pct:.0f}%  [{result.water.status}]"),
            (result.food.status,          f"Food : {result.food.pct:.0f}%  [{result.food.status}]"),
        ]
